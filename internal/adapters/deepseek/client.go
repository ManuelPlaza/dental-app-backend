...
func NewClient() *Client {
	...
	sim := apiKey == ""
	if sim {
		log.Println("⚠️  [DeepSeekClient] DEEPSEEK_API_KEY no configurada — modo simulación activo")
	}
...
func (c *Client) Chat(systemPrompt string, messages []Message) (string, error) {
	if c.sim {
		log.Printf("[DeepSeekClient:sim] Chat llamado con %d mensajes", len(messages))
		return "Modo simulación activo. Configure DEEPSEEK_API_KEY en las variables de entorno para activar el asistente.", nil
	}
...
// doRequest envía la solicitud a DeepSeek y retorna la respuesta parseada.
func (c *Client) doRequest(messages []Message, tools []Tool) (*chatResponse, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		log.Printf("[DeepSeekClient] error status=%d code=%s msg=%s", resp.StatusCode, errBody.Error.Code, errBody.Error.Message)
		if resp.StatusCode == 429 {
			return nil, fmt.Errorf("rate_limit: %s", errBody.Error.Message)
		}
		return nil, fmt.Errorf("DeepSeek status %d: %s", resp.StatusCode, errBody.Error.Message)
	}
